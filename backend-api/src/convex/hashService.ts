...
export class HashService {
  /**
   * Store a new webserver state hash
  ...
  async storeHash(hashData: CreateHashRequest): Promise<string> {
    try {
      ...
      throw new Error(
        `Failed to store hash: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async getHashesByRoom(roomId: string): Promise<Hash[]> {
    try {
      ...
      throw new Error(
        `Failed to get hashes: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   * @param roomId Room ID
   * @param tick Webserver tick number
   * @returns Hash data or null if not found
   */
  async getHashByRoomAndTick(
    roomId: string,
    tick: number
  ): Promise<Hash | null> {
    try {
      ...
      const hash = await client.query("hashes:getByRoomAndTick", {
        roomId,
        tick,
      });
      ...
      throw new Error(
        `Failed to get hash: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async deleteHashesByRoom(roomId: string): Promise<boolean> {
    try {
      ...
      throw new Error(
        `Failed to delete hashes: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
}
